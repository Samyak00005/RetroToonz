...
import { getAllShows, getEpisodesForShow, getShowById } from "../../services/contentService.js";
...
function VideoPlayerPage() {
      ...
      <div className="rt-page flex min-h-screen flex-col">
        ...
        <main className="rt-content rt-page-pad flex flex-1 items-center justify-center">
          <div className="rt-surface max-w-lg p-8 text-center">
            ...
            <p className="mt-3 text-white/60">This show is missing or the video link is no longer valid.</p>
  ...
  const requestedEpisodeId = routeEpisodeId || legacyEpisodeId || stateEpisodeId || allEpisodes[0]?.episodeId;

  let currentEpisodeIndex = allEpisodes.findIndex((episode) => String(episode.episodeId) === String(requestedEpisodeId));
  if (currentEpisodeIndex < 0 && allEpisodes.length) currentEpisodeIndex = 0;

  const currentEpisode = currentEpisodeIndex >= 0 ? allEpisodes[currentEpisodeIndex] : null;
  const canonicalPath = currentEpisode ? buildWatchPath(currentShow.id, currentEpisode.episodeId) : buildWatchPath(currentShow.id);

  if (currentEpisode && (location.pathname !== canonicalPath || location.search)) {
    return <Navigate to={canonicalPath} replace />;
  }

  const canGoPrevious = currentEpisodeIndex > 0;
  const canGoNext = currentEpisodeIndex >= 0 && currentEpisodeIndex < allEpisodes.length - 1;
      ...
      <main className="relative flex-1 overflow-hidden">
        <AnimatePresence mode="wait">
          ...
          >
            ...
            <ShowInfo currentShow={currentShow} currentEpisode={currentEpisode} />
            ...
            {upcomingShows.length > 0 && (
              <div className="pb-4 sm:pb-8">
                <ShowSection sectionTitle="You may also like" shows={upcomingShows} sectionKey="up-next" showMoreTo="/all-shows" />
              </div>
            )}
          </motion.div>
        </AnimatePresence>
